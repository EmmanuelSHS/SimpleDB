...
public class TransactionTest extends TestUtil.CreateHeapFile {
  ...
  public void testTransactionComplete(boolean commit) throws Exception {
    ...
    // test whether successfully written in buffer
    /*
    HeapPage p1 = (HeapPage) bp.getPage(tid1, p2, Permissions.READ_WRITE);
    boolean f = false;
    Iterator<Tuple> it1 = p1.iterator();
    while (it1.hasNext()) {
    	Tuple tp = (Tuple) it1.next();
        IntField f0 = (IntField) tp.getField(0);
        IntField f1 = (IntField) tp.getField(1);
